...
chirprdb.storeupdate = (gmail, name, address, latitude, longitude, category, phone, information, newname) => {
  return new Promise((resolve, reject) => {
    pool.query(`update 자영업자할인 set gmail=?, name=?, address=?, latitude=?, longitude=?, category=?, phone=?, information=? where gmail=? and name=?`, [gmail, newname, address, latitude, longitude, category, phone, information, gmail, name], (err, results) => {
      if(err) {
        return reject(err);
      }
      return resolve("done");
